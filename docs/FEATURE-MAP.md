...
1bb. `src/presentation/honeypotPathMapping.go` — `honeypotPayloadSpec` struct and `honeypotPayloadSpecs` slice with 25 entries mapping URL paths to base64-encoded .bin filenames and MIME types; `findPayloadSpec` lookup by URL path
...
9. Fake payloads: 25 base64-encoded .bin files embedded via `//go:embed honeypot/payloads/*.bin`, decoded at construction time for active paths only
